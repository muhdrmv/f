...
import {extractSessionKeystrokes, renderSessionOcr, renderSessionVideo} from "./SessionActions";
...
const SessionView = ({licenseInfo}) => {
    ...
    useEffect(() => {
        if (data?.sessions_by_pk) {
            ...
            if(sessionData?.meta?.transparentMode){
                history.push(`/dashboard/sessions-history/${id}/view-transparent`);
            }
    ...
    const handleClickRenderVideo = async () => {
        if (window.confirm('Are you sure you want to start rendering this session?')) {
            ...
            const result = await renderSessionVideo(mutateSessionVideo, id);
            console.log(result);
        }
    }
