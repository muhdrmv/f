...
const SessionRow = ({row}) => {
    ...
    const handleViewClick = (id, transparentMode) => {
        if(transparentMode) history.push('/dashboard/sessions-history/' + id+'/view-transparent');
        else history.push('/dashboard/sessions-history/' + id+'/view');
        ...
        <>
            <TableCell align="right" style={{width: 250, paddingRight: 0}}>
                ...
                <Tooltip title="Details">
                    <IconButton onClick={e => handleViewClick(row.id, row?.meta?.transparentMode)}
                                color="primary"
                                aria-label="edit">
