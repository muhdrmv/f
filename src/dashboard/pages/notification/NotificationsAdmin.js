...
const GET_ADMIN_NOTIFICATIONS_FILTERED = gql`
...
const Notifications = ({loggedInUser, countNotifications, setNotificationsCount}) => {
    ...
    const [query, setQuery] = useState(GET_ALL_NOTIFICATIONS);
    ...
    const enableFilterQuery = (filterText) => {
        ...
        setQuery(GET_ADMIN_NOTIFICATIONS_FILTERED);
        setQueryVars({filter: filterText})
    }

    const disableFilterQuery = () => {
       setQuery(GET_ALL_NOTIFICATIONS);
    }
